...
export default function DecryptedText({
  ...
  animateOn = 'hover',
  clickMode = 'once',
  trigger = false, // 외부 수동 트리거 플래그
  forceReveal = false, // 외부 강제 즉시 해독 플래그
  ...props
}) {
  ...
  /* Force External Instant Reveal */
  useEffect(() => {
    if (forceReveal) {
      resetToPlainText();
    }
  }, [forceReveal, resetToPlainText]);

  /* Custom External Trigger Observer */
  useEffect(() => {
    if (animateOn === 'custom' && trigger && !hasAnimated) {
      triggerDecrypt();
      setHasAnimated(true);
    }
  }, [animateOn, trigger, hasAnimated, triggerDecrypt]);
  ...
  useEffect(() => {
    if (animateOn === 'click' || animateOn === 'custom') {
      encryptInstantly();
    } else {
      setDisplayText(text);
      setIsDecrypted(true);
